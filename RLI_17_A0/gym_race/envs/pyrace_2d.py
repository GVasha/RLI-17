...
class PyRace2D:
    def __init__(self, is_render = True, car = True, mode = 0):
        ...
        if car:
            ...
            # Seed cur_distance so the first shaping reward is not a large spike
            self.car.cur_distance = get_distance(check_point[self.car.current_check], self.car.center)
        self.game_speed = 60*0 # as fast as possible...
        self.is_render = is_render
        self.mode = mode # 0: normal, 1:dark, 2: normal (force display)

    def action(self, action, extended_actions=False):
        if action == 0:
            self.car.speed += 2
        elif action == 1:
            self.car.angle += 5
            ...
            self.car.angle -= 5
        elif extended_actions and action == 3:
            self.car.speed -= 3
    ...
    def evaluate(self, reward_mode="v1"):
        if reward_mode == "v2":
            reward = self.car.prev_distance - self.car.cur_distance
        ...
        elif self.car.goal:  # completed full lap
            reward = 10000
        elif self.car.check_flag:  # passed a checkpoint
            self.car.check_flag = False
            reward = 500 - 0.5 * self.car.time_spent
            self.car.time_spent = 0
        else:
            reward = self.car.prev_distance - self.car.cur_distance
        return reward
    ...
    def observe(self, continuous=False):
